...
All 28 languages in the app's language selector have document and Naru-question interface copy, including upload/camera controls, online-account recovery, errors, and suggested questions. The new processing-consent notice currently supports Korean, Chinese, Japanese, and English, with English fallback for other languages. Arabic, Urdu, and Persian use right-to-left document panels and chat. Switching the interface language updates the default translation target and the language of subsequent Naru replies; document text and existing conversation messages remain intact. Additional feature locale packs live in `src/documentLocales/`; coverage tests fail if a configured language or required message is missing.

- Supported inputs: JPEG, PNG, PDF, and UTF-8 TXT, up to 10 MiB and 20,000 extracted characters per document.
- Select or capture a file locally, then explicitly consent before uploading for OCR, translation, or questions. The notice identifies OpenAI and sensitive health/identity data. New files start unchecked, and consent withdrawal is synchronized between the document and its Naru conversation. Both client and server reject processing without `processingConsent: true` (multipart uses `"true"`).
- Photos use multilingual text recognition. PDFs are sent as OpenAI file inputs so scanned pages can also be read. TXT is decoded without a model call.
...
- Documents are temporary by default: original file, OCR text, translation, and document conversation remain in browser memory, without R2 or D1 document writes. Refreshing, signing out, replacing a temporary attachment, or closing it clears that temporary document. The authenticated Worker processes its translation and questions statelessly.
- Saving history is a separate unchecked option (`saveToHistory: "true"` on upload). Only then are original files private R2 objects under `medical-documents/`, using the existing `RECORDINGS` binding, and D1 stores account-owned metadata and text. Saved history supports reopening, retranslating, and deletion. Reopening a saved document requires fresh processing consent.
- Document OCR, translation, and questions require `OPENAI_API_KEY` and use the existing `OPENAI_MODEL`. There is no automatic cross-provider fallback for document API requests. Responses use `store: false`; this does not promise zero provider retention, so the consent notice links to OpenAI's separate data policy. Original text, numbers, and completion checks remain intact. Repeating an unchanged saved translation reuses that account's result without another model call.
...
Document regression checks: `npm test`, `npm run visual:documents`, `npm run visual:document-naru`, `npm run visual:document-locales`, and `npm run visual:document-consent` (start the Vite server first). The locale check walks every configured language at mobile width, including error recovery and switching languages during a document conversation. Browser checks use synthetic documents and mocked API responses, not real patient records. The consent check additionally verifies temporary-document handling and that browser storage stays clear. Set `DOCUMENT_LIVE_CHECK=1` only to also test the local backend and configured OpenAI provider with synthetic PNG/PDF files; this creates a synthetic local account, signs it out afterward, and can incur API usage.
