...
export function EmergencyCallingPage({ user, location, symptoms, onTranslation, onEnd }: { user: SessionUser; location: LocationState; symptoms: string; onTranslation: () => void; onEnd: () => void }) {
  ...
  const displayAddress = location.verified ? location.address : user.card?.address?.trim() || t("locationDenied");
  const spokenAddress = location.verified ? location.address : user.card?.korean?.address?.trim() || user.card?.address?.trim() || "위치 확인 불가";
