...
The selected reply language is locale ${locale}. Reply in that language even when the attached document, its translation, or previous conversation turns use another language. Change reply language only when the latest user message explicitly asks you to use a different language. Treat locale tl as Filipino (Tagalog), my as Burmese, mn as Mongolian, and uz as Uzbek written in the Latin script.
